...
class SodEnv(py_environment.PyEnvironment):
  ...
  def __init__( self
    ...
    if ((self.env_launcher == 'local') and (n_procs != 1)):
      ...
      printWarning("To run environments in parallel with MPI, use env_launcher='mpirun'")
      n_procs = 1
    ...
    self.sod2d = self._start_flexi(self.exp,self.n_procs,self.n_envs)
  ...
  def stop(self):
    ...
    if self.exp:
      self._stop_sod2d_instances(self.exp)
  ...
  def _start_sod2d(self,exp,n_procs,n_envs):
    ...
    if (ranks_needed > ranks_avail):  # are there enough free ranks?
      raise ValueError(f"Only {ranks_avail} ranks are available, but {ranks_needed} would be required "+
                    "to start {n_envs} on {n_procs} each.")
